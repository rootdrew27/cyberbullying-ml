...
if __name__ == '__main__':
    ...
    default_xgb_param_dists = {
        ...
        'classifier__reg_alpha': loguniform(1e-2, 5),
        'classifier__reg_lambda': loguniform(1e-2, 5),
    ...
    param_dist['classifier__scale_pos_weight'] = RandomRatio(ratio=ratio, low=-0.6, high=0.27)

    # default pipeline
    pipeline = Pipeline([
        ('vectorizer', 'passthrough'),
        ('classifier', XGBClassifier(objective='binary:logistic', nthreads=-1))
    ])
    ...
    param_dist['classifier__class_weights'] = RandomRatio2(ratio=ratio, low=-0.6, high=0.27)
